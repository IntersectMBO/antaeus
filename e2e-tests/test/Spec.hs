...
tests :: IORef [TestResult] -> IORef [TestResult] -> IORef [TestResult] -> IORef [TestResult] -> TestTree
tests pv6ResultsRef pv7ResultsRef pv8ResultsRef pv9TestsRef = testGroup "Plutus E2E Tests" [
  ...
  testProperty "Babbage PV9 Tests" (pv9Tests pv9TestsRef)
...
pv9Tests resultsRef = CTN.integration . HE.runFinallies . U.workspace "." $ \tempAbsPath -> do
...
runTestsWithResults = do
  ...
  allRefs@[pv6ResultsRef, pv7ResultsRef, pv8ResultsRef, pv9ResultsRef] <- traverse newIORef [[], [], [], []]

  -- Catch the exception returned by defaultMain to proceed with report generation
  _ <- try (defaultMain $ tests pv6ResultsRef pv7ResultsRef pv8ResultsRef pv9ResultsRef) :: IO (Either SomeException ())
  ...
  let
    ...
    pv9TestSuiteResult = TestSuiteResults "Babbage PV9 Tests" pv9Results

  -- Use 'results' to generate custom JUnit XML report
  let xml = testSuitesToJUnit [pv6TestSuiteResult, pv7TestSuiteResult, pv8TestSuiteResult, pv9TestSuiteResult]
